    ...
    UUPSUpgradeable 
{
    ...
    function _getBlogStorage() private pure returns (BlogStorage storage $) {
        assembly { $.slot := BlogStorageLocation }
    }
    ...
    function __Blog_init(address, uint256, string calldata)
        public
        override
        initializer
    {
    ...
    function contractName() external pure override returns (string memory) { return "Blog"; }
    ...
    function isPremiumHolder(address who) external view returns (bool) {
        return balanceOf(who, uint256(PREMIUM)) > 0;
    }

    // === MONEY IN ===
    receive() external payable { emit FundsReceived(_msgSender(), msg.value); }
    ...
    function modifyURI(string memory newuri) external override onlyOwner { _setURI(newuri); }

    function updatePremiumFee(uint256 newFee) external override onlyOwner {
    ...
    function mintPremium(string calldata tokenURI)
        public
        payable
        override
        whenNotPaused
    ...
    function withdraw(address payable des) external override onlyOwner whenNotPaused nonReentrant {
